...
describe 'ExcelCSV' do
  ...
  describe 'read' do
    ...
    # If a file generated by this library is opened in Mac Numbers or other programs, then re-exported, this will be the
    # result:
    it "should read in a spreadsheet with a sep=, special first line, followed by a string of ','s" do
      rows = ExcelCSV.read write_csv <<-EOF.strip_heredoc
        sep=,,,,,
        Ad ID,Custom Code,Name,Title,Description,Price
      EOF

      expect(rows).to eq([["Ad ID", "Custom Code", "Name", "Title", "Description", "Price"]])
    end
